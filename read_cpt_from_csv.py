...
class read_cpts_from_csv():
    def __init__(self,net):
        ...
        self.df.columns = ['S','YF','AN','PP','G','AD','BED','CO','F','AL','CA','LC']

    def find_prob0(self,var_nosign,variables,conditions=None,new_cond=None):
        sign = variables[0]
        count = self.df[var_nosign].value_counts()
        count = count[sign]
        return (count+1)/(self.row+2)

    def find_prob1(self,var_nosign,variables,conditions=None,new_cond=None):
        sign = variables[0]
    ...
    def find_prob2(self,var_nosign,variables,conditions=None,new_cond=None):
    ...
    def read_cpts(self):
        ...
        for i in range(len(variables)):
            ...
            print(var_nosign[i],variables[i],new_cond[i],conditions[i],con[i])


        size = len(var_nosign)
        for i in range(size):
            if conditions[i] is  None:
                self.net[var_nosign[i]][con[i]] = self.find_prob0(var_nosign[i],variables[i])
                print(var_nosign[i],con[i],self.net[var_nosign[i]][variables[i]])
            else:
                self.net[var_nosign[i]][con[i]] = self.find_prob1(var_nosign[i],variables[i],conditions[i], new_cond[i])
                print(var_nosign[i],con[i],self.net[var_nosign[i]][con[i]])
